...
preserve
	...
	* trimming (the 2nd and 98th percentiles)
	drop if beta > 1.9697362
	drop if beta < -2.20831236
	...
	* mean and median
	...
	graph save hist_beta_rural.gph, replace
restore

preserve
	collapse beta phi, by(hh)
	
	drop if phi > 0.22078344
	drop if phi < -0.21899772
	
	sum phi, d
	 
	hist phi, title("Phi across households", color(black)) ///
	xtitle ("Phi") graphregion(color(white)) bcolor(navy)
	graph save hist_phi_rural.gph, replace
restore
...
gen inc_g = 0
replace inc_g = 1 if nit <= 445
replace inc_g = 2 if nit > 445 & nit <= 889
replace inc_g = 3 if nit > 889 & nit <= 1334
replace inc_g = 4 if nit > 1334 & nit <= 1778
replace inc_g= 5 if nit > 1778 & nit <= 2223
...
gen beta_g = 0
replace beta_g = 1 if nit <= 445
replace beta_g = 2 if nit > 445 & nit <= 889
replace beta_g = 3 if nit > 889 & nit <= 1334
replace beta_g = 4 if nit > 1334 & nit <= 1778
replace beta_g = 5 if nit > 1778 & nit <= 2223
